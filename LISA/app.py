...
def parse_args(args):
    ...
    # https://huggingface.co/xinlai/LISA-13B-llama2-v1
...
def inference(input_str, input_image):
    ...
    if not re.match(r"^[A-Za-z ,.!?\'\"]+$", input_str) or len(input_str) < 1:
        ...
        ## error happened
        output_image = cv2.imread("./error_happened.png")[:, :, ::-1]
    ...
    if save_img is not None:
        ...
        ## no seg output
        output_image = cv2.imread("./no_seg_out.png")[:, :, ::-1]
    # Added pred_mask to support downstream processing
    return output_image, pred_mask, output_str
...
demo = gr.Interface(
    ...
    outputs=[
        ...
        # Added Numpy Array Output to expose the raw mask for further processing
        gr.Numpy(label="Numpy Array Output"),
